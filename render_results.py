...
def parse_args():
    ...
    parser.add_argument('--pred_dir', default=None, type=str,
                        help='Optional directory containing .npy predictions. If set, render from this directory (e.g., <run>/predictions_foldpath).')
    parser.add_argument('--top_k_paths', default=None, type=int,
                        help='For FoldPath-style outputs: keep only top-K paths by confidence before rendering (requires conf_pred).')
    parser.add_argument('--conf_thresh', default=None, type=float,
                        help='For FoldPath-style outputs: keep only paths with conf >= threshold before rendering (requires conf_pred).')
...
def main():
    ...
    if args.pred_dir is not None:
        ...
        if not os.path.isabs(pred_dir):
            pred_dir = os.path.join(args.run, pred_dir)
        assert os.path.isdir(pred_dir), f'pred_dir does not exist: {pred_dir}'
        filenames = sorted(glob.glob(os.path.join(pred_dir, '*.npy')))
        if args.split is not None:
            filenames = [f for f in filenames if f'_{args.split}_' in os.path.basename(f) or f'_{args.split}' in os.path.basename(f)]
    else:
    ...
    for postprocess_flag in postprocess_flags:
        for j, file in enumerate(filenames):  # iterate over different batches, train/test, best/last
            ...
            traj = data.get('traj') if 'traj' in data else data.get('traj_as_pc')
            stroke_ids = data.get('stroke_ids') if 'stroke_ids' in data else data.get('stroke_ids_as_pc')
            ...
            conf_pred = data.get('conf_pred') if 'conf_pred' in data else None
            # FoldPath-style optional filtering for visualization
            if conf_pred is not None and stroke_ids_pred is not None and (args.top_k_paths is not None or args.conf_thresh is not None):
                def _keep_path_ids(conf_vec):
                    conf_vec = np.asarray(conf_vec).reshape(-1)
                    ids = np.arange(len(conf_vec))
                    if args.conf_thresh is not None:
                        ids = ids[conf_vec >= float(args.conf_thresh)]
                    if args.top_k_paths is not None and len(ids) > int(args.top_k_paths):
                        order = np.argsort(conf_vec[ids])[::-1][:int(args.top_k_paths)]
                        ids = ids[order]
                    return set([int(i) for i in ids])

                for b in range(traj_pred.shape[0] if hasattr(traj_pred, 'shape') else len(traj_pred)):
                    b_conf = conf_pred[b] if (hasattr(conf_pred, '__len__') and not np.isscalar(conf_pred)) else conf_pred
                    keep_ids = _keep_path_ids(b_conf)
                    b_sid = stroke_ids_pred[b]
                    b_trp = traj_pred[b]
                    mask = np.isin(np.asarray(b_sid).astype(np.int64), list(keep_ids))
                    traj_pred[b] = np.asarray(b_trp)[mask]
                    stroke_ids_pred[b] = np.asarray(b_sid)[mask]
