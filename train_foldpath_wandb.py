...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--wandb_project", type=str, default="foldpath", help="WandB project name")
    ...
    p.add_argument("--log_interval", type=int, default=10, help="Log training metrics every N batches")
    return p.parse_args()
...
def main() -> None:
    ...
    if args.use_wandb:
        wandb.init(
            ...
            name=args.wandb_run_name or f"{args.dataset}_{args.activation}",
            config=vars(args)  # Log all arguments
        )
        # Log model configuration
        wandb.config.update({"model_cfg": asdict(FoldPathConfig(
    ...
    with open(config_path, "w", encoding="utf-8") as f:
        import json
        config_dict = {"args": vars(args), "model_cfg": asdict(model_cfg)}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Extract loss values (they might be tensors or floats)
            loss_value = logs['loss'] if isinstance(logs['loss'], float) else logs['loss'].item()
            loss_points_value = logs['loss_points'] if isinstance(logs['loss_points'], float) else logs['loss_points'].item()
            loss_conf_value = logs['loss_conf'] if isinstance(logs['loss_conf'], float) else logs['loss_conf'].item()
            
            # Accumulate epoch statistics
            epoch_train_loss += loss_value
            epoch_loss_points += loss_points_value
            epoch_loss_conf += loss_conf_value
            num_batches += 1
            
            # Update progress bar
            pbar.set_postfix({
                "loss": f"{loss_value:.4f}", 
                "lp": f"{loss_points_value:.4f}", 
                "lc": f"{loss_conf_value:.4f}"
            })
            
            # Log batch metrics to WandB
            if args.use_wandb and (batch_idx + 1) % args.log_interval == 0:
                wandb.log({
                    "train/loss_batch": loss_value,
                    "train/loss_points_batch": loss_points_value,
                    "train/loss_conf_batch": loss_conf_value,
        ...
        avg_loss_points = epoch_loss_points / num_batches if num_batches > 0 else 0
        avg_loss_conf = epoch_loss_conf / num_batches if num_batches > 0 else 0
        ...
        with torch.no_grad():
            for batch in tqdm(te_loader, desc="eval", leave=False):
                ...
                # Extract loss values for test
                test_loss_value = float(loss.detach().cpu())
                test_loss_points_value = logs['loss_points'] if isinstance(logs['loss_points'], float) else logs['loss_points'].item()
                test_loss_conf_value = logs['loss_conf'] if isinstance(logs['loss_conf'], float) else logs['loss_conf'].item()
                
                te_losses.append(test_loss_value)
                te_loss_points.append(test_loss_points_value)
                te_loss_conf.append(test_loss_conf_value)
        
        mean_te = sum(te_losses) / max(1, len(te_losses))
        mean_te_points = sum(te_loss_points) / max(1, len(te_loss_points))
        mean_te_conf = sum(te_loss_conf) / max(1, len(te_loss_conf))
        
        print(f"Epoch {epoch}: "
              f"train_loss={avg_train_loss:.6f}, "
              f"test_loss={mean_te:.6f} "
        ...
        if args.use_wandb:
            wandb.log({
                ...
                "train/loss_points": avg_loss_points,
                "train/loss_conf": avg_loss_conf,
                "test/loss": mean_te,
                "test/loss_points": mean_te_points,
                "test/loss_conf": mean_te_conf,
                "learning_rate": sched.get_last_lr()[0],
            })

        # Save checkpoint
        checkpoint_path = os.path.join(args.out_dir, "checkpoints", "last.pth")
        save_checkpoint(checkpoint_path, model, optim, epoch)
        
        # Upload checkpoint to WandB
        if args.use_wandb and epoch % 10 == 0:  # Upload every 10 epochs to avoid too many files
            wandb.save(checkpoint_path, base_path=os.path.dirname(checkpoint_path))
        
        if epoch % 50 == 0:
            epoch_checkpoint_path = os.path.join(args.out_dir, "checkpoints", f"epoch_{epoch:04d}.pth")
            save_checkpoint(epoch_checkpoint_path, model, optim, epoch)
            if args.use_wandb:
                wandb.save(epoch_checkpoint_path, base_path=os.path.dirname(epoch_checkpoint_path))
