#!/usr/bin/env python3
"""
Visualize FoldPath predictions with 3D point clouds.
Visualizes predicted trajectories alongside ground truth and 3D object meshes.
"""
...
def load_obj_vertices(obj_path):
    ...
    Returns:
        numpy.ndarray: Array of vertex coordinates (N, 3) or None if failed
    """
    vertices = []
...
def denormalize_points(points, norm_params):
    ...
    Args:
        points: Input points array (N, 3+)
        norm_params: Dictionary with 'center' and 'scale' keys
        
    Returns:
        numpy.ndarray: Denormalized points
    """
...
def create_comparison_visualization(vertices, pred_trajectories, gt_trajectories,
                                   sample_name, output_path, top_k=3):
    """
    Create comparison visualization of predictions vs ground truth.
    
    Args:
        vertices: Mesh vertices for background (N, 3)
        ...
        top_k: Number of top trajectories to visualize
    """
    try:
        ...
        # Display predicted trajectories (top_k)
        pred_colors = ['red', 'orange', 'green', 'purple', 'brown']
        for i in range(min(top_k, len(pred_trajectories))):
        ...
        # Display ground truth trajectories
        gt_colors = ['blue', 'cyan', 'magenta']
        for i in range(min(top_k, len(gt_trajectories))):
            traj = gt_trajectories[i]
            if len(traj) > 0:
                color = gt_colors[i % len(gt_colors)]
                ax.plot(traj[:, 0], traj[:, 1], traj[:, 2],
                       color=color, alpha=0.6, linewidth=2.0, 
                       linestyle='--', label=f'GT Path {i}')
...
def create_confidence_visualization(vertices, trajectories, titles,
                                   colors, output_path, sample_name):
    """
    Create visualization of trajectories colored by confidence.
    
    Args:
        vertices: Mesh vertices for background
        trajectories: List of trajectories to visualize
        titles: List of titles for each trajectory
    ...
    try:
        ...
        if vertices is not None and len(vertices) > 0:
            ...
            ax.scatter(vertices_display[:, 0], vertices_display[:, 1],
                      ...
                      alpha=0.2, marker='.')
            logging.info(f"Displaying {len(vertices_display)} mesh points")
        ...
        for i, (traj, title, color) in enumerate(zip(trajectories, titles, colors)):
            if traj is not None and len(traj) > 0:
                ...
                # Draw sparse trajectory points
                if len(traj) > 20:
                    step = len(traj) // 20
                    traj_points = traj[::step]
                else:
                    traj_points = traj
                
                ax.scatter(traj_points[:, 0], traj_points[:, 1], 
                          traj_points[:, 2], c=color, s=20, 
                          alpha=0.6, marker='o')
...
def main():
    parser = argparse.ArgumentParser(description='Visualize FoldPath predictions with 3D point clouds')

    parser.add_argument('--pred_dir', required=True, type=str, help='Directory containing all_predictions.npy')
    parser.add_argument('--normalized_root', default='/fileStore/windows-v2-normalized', type=str, help='Directory containing normalized OBJ files')
    parser.add_argument('--sample_dirs', nargs='+', default=['1_wr1fr_1'], help='Sample directory names')
    parser.add_argument('--output_dir', default=None, type=str, help='Output directory for visualizations')
    parser.add_argument('--max_samples', default=1, type=int, help='Maximum number of samples to visualize')
    parser.add_argument('--top_k_paths', default=3, type=int, help='Number of top paths to visualize')
    ...
    logging.info("=" * 60)
    logging.info("FoldPath Point Cloud Visualizer")
    logging.info("=" * 60)
    ...
    logging.info(f"Top K paths: {args.top_k_paths}")
    logging.info("=" * 60)
    ...
    try:
        ...
        logging.info(f"Loaded predictions: {traj_pred.shape}")
        ...
        for sample_idx in range(num_samples):
            sample_dir = args.sample_dirs[sample_idx % len(args.sample_dirs)]
            ...
            pred_trajectories = []
            for path_idx in range(min(args.top_k_paths, num_queries)):
                try:
            ...
            if traj_gt is not None:
                for path_idx in range(min(args.top_k_paths, traj_gt.shape[1])):
                    try:
            ...
            if conf_pred is not None and sample_idx < conf_pred.shape[0]:
                ...
                # Sort by confidence
                top_indices = np.argsort(conf_scores)[::-1][:args.top_k_paths]
                top_trajectories = []
                top_titles = []
